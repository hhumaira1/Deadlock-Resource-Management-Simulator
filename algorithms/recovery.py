...
def terminate_process(pid: int, system_state: SystemState) -> Tuple[bool, str]:
    ...
    resources_str = ", ".join([f"R{i}[{resources_held[i]}]" for i in range(len(resources_held)) if resources_held[i] > 0])
    ...
    if method == "terminate":
        ...
        while remaining_deadlocked:
            ...
            if not deadlock_exists:
                # Deadlock resolved - NOTE: Don't reset states, retry logic checks Request Matrix
